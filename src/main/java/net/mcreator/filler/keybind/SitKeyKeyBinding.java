...
package net.mcreator.filler.keybind;
...
import net.mcreator.filler.procedures.SitKeyOnKeyPressedProcedure;
...
public class SitKeyKeyBinding extends FillerModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	...
	public void initElements() {
		keys = new KeyBinding("key.filler.sit_key", GLFW.GLFW_KEY_H, "key.categories.movement");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					FillerMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				SitKeyOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
